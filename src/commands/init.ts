...
const CONFIG_TEMPLATE_ZH = `[vault]
name = "My Wiki"
language = "zh"
...
export const initCommand = new Command('init')
  ...
  .option('--existing', 'adopt an existing wiki directory without overwriting files')
  .option('--language <lang>', 'vault language (default: en, or zh if existing wiki detected)', '')
  .action((directory: string, opts: { existing?: boolean; language?: string }) => {
    const targetDir = join(process.cwd(), directory);

    // Check if already initialized
    if (!opts.existing && findVaultRoot(targetDir)) {
    ...
    // Detect language for existing wikis
    let lang = opts.language || 'en';
    if (opts.existing && !opts.language) {
      if (existsSync(paths.wiki)) {
        try {
          const files = readdirSync(paths.wiki, { recursive: true, withFileTypes: false });
          for (const f of files) {
            if (typeof f === 'string' && f.endsWith('.md')) {
              const content = readFileSync(join(paths.wiki, f), 'utf-8');
              if (/[一-鿿]/.test(content)) {
                lang = 'zh';
                break;
              }
    ...
    const configTemplate = lang === 'zh' ? CONFIG_TEMPLATE_ZH : CONFIG_TEMPLATE;
    ...
    // Install skills first (before vault marker)
    ...
    if (opts.existing) {
      console.log('(adopted existing wiki directory)');
    }
    console.log('');
    console.log('Files:');
    console.log(`  ${created} created, ${skipped} already existed (preserved)`);
    ...
    console.log('  1. Edit wiki-purpose.md (or purpose.md) to define your wiki\'s scope');
    console.log('  2. Edit wiki-schema.md (or schema.md) to set naming conventions');
